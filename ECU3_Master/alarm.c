/*
 * ---------------------------------------------------------------------------
 *  ECU-3 Alarm module : status LEDs, buzzer and Timer1 red-LED blink
 *  Green LED  P1.16 : SOLID ON when safe
 *  Red LED    P1.17 : blinks at 500 ms intervals while an alarm is active
 *  Buzzer     P1.18 : solid ON while an alarm is active
 *
 *  The blink rate is generated by a Timer1 match interrupt so that it never
 *  blocks the other MCU operations (CAN reception, LCD refresh, ...).
 * ---------------------------------------------------------------------------
 */
#include "ecu3.h"
