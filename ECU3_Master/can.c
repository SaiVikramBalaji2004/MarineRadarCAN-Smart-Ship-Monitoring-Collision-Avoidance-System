/*
 * ---------------------------------------------------------------------------
 *  ECU-3 CAN1 receive driver : acceptance filter + interrupt reception
 *  CAN1 : RD1 = P0.25 (PINSEL1[19:18] = 01), TD1 = dedicated pin on LPC2129
 *
 *  The acceptance filter is programmed to accept only the individual
 *  standard identifiers 0x101 .. 0x107 (all other bus traffic is dropped).
 *  Received frames update the shared telemetry variables in the ISR.
 * ---------------------------------------------------------------------------
 */
#include "ecu3.h"
...
/* Individual standard-ID table entry : CONTROLLER# bits[15:13],
   DISABLE bit[12], identifier bits[10:0] (see UM10114 Fig.67).          */
#define AF_STD_ENTRY(controller, id) \
    (((unsigned long)(controller) << 13) | (unsigned long)(id))

/* CAN controller number as used inside the LUT tables.
   1 = CAN1 on no-suffix /00 LPC2129 devices.
   Note: on /01 devices the controllers are numbered 0..n-1, so change to 0. */
